...
package com.shaun.data_structure_algorithm.textbook_algorithm_version4.official;
...
public class TwoPersonZeroSumGame {
    ...
    private edu.princeton.cs.algs4.LinearProgramming lp;   // linear program solver
    ...
    private boolean isPrimalFeasible() {
        ...
        for (int j = 0; j < n; j++) {
            if (x[j] < 0) {
                edu.princeton.cs.algs4.StdOut.println("row vector not a probability distribution");
                edu.princeton.cs.algs4.StdOut.printf("    x[%d] = %f\n", j, x[j]);
                return false;
            }
            sum += x[j];
        }
        if (Math.abs(sum - 1.0) > EPSILON) {
            edu.princeton.cs.algs4.StdOut.println("row vector x[] is not a probability distribution");
            edu.princeton.cs.algs4.StdOut.println("    sum = " + sum);
            return false;
        }
        return true;
    }
    ...
    private boolean isDualFeasible() {
        ...
        for (int i = 0; i < m; i++) {
            if (y[i] < 0) {
                edu.princeton.cs.algs4.StdOut.println("column vector y[] is not a probability distribution");
                edu.princeton.cs.algs4.StdOut.printf("    y[%d] = %f\n", i, y[i]);
                return false;
            }
            sum += y[i];
        }
        if (Math.abs(sum - 1.0) > EPSILON) {
            edu.princeton.cs.algs4.StdOut.println("column vector not a probability distribution");
            edu.princeton.cs.algs4.StdOut.println("    sum = " + sum);
            return false;
        }
        return true;
    ...
    private boolean isNashEquilibrium(double[][] payoff) {
        ...
        if (Math.abs(opt1 - value) > EPSILON) {
            edu.princeton.cs.algs4.StdOut.println("Optimal value = " + value);
            edu.princeton.cs.algs4.StdOut.println("Optimal best response for column player = " + opt1);
            return false;
        }
        ...
        if (Math.abs(opt2 - value) > EPSILON) {
            edu.princeton.cs.algs4.StdOut.println("Optimal value = " + value);
            edu.princeton.cs.algs4.StdOut.println("Optimal best response for row player = " + opt2);
            return false;
        }
    ...
    private static void test(String description, double[][] payoff) {
        ...
        for (int j = 0; j < n-1; j++)
            edu.princeton.cs.algs4.StdOut.printf("%8.4f, ", x[j]);
        edu.princeton.cs.algs4.StdOut.printf("%8.4f]\n", x[n-1]);

        edu.princeton.cs.algs4.StdOut.print("y[] = [");
        for (int i = 0; i < m-1; i++)
            edu.princeton.cs.algs4.StdOut.printf("%8.4f, ", y[i]);
        edu.princeton.cs.algs4.StdOut.printf("%8.4f]\n", y[m-1]);
