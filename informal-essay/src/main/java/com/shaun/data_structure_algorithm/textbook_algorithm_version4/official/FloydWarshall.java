...
public class FloydWarshall {
    ...
    public Iterable<edu.princeton.cs.algs4.DirectedEdge> negativeCycle() {
        for (int v = 0; v < distTo.length; v++) {
            ...
            if (distTo[v][v] < 0.0) {
                int V = edgeTo.length;
                edu.princeton.cs.algs4.EdgeWeightedDigraph spt = new EdgeWeightedDigraph(V);
                for (int w = 0; w < V; w++)
                    if (edgeTo[v][w] != null)
                        spt.addEdge(edgeTo[v][w]);
                edu.princeton.cs.algs4.EdgeWeightedDirectedCycle finder = new EdgeWeightedDirectedCycle(spt);
