...
public class TopologicalX {
    private edu.princeton.cs.algs4.Queue<Integer> order;     // vertices in topological order
     ...
     * @param G the digraph
     */
    public TopologicalX(edu.princeton.cs.algs4.Digraph G) {
        ...
        // initialize 
        ranks = new int[G.V()]; 
        order = new edu.princeton.cs.algs4.Queue<Integer>();
        int count = 0;
    ...
    public static void main(String[] args) {
        ...
        for (int v = 0; v < G1.V(); v++)
            for (int w : G1.adj(v))
                G2.addEdge(new edu.princeton.cs.algs4.DirectedEdge(v, w, 0.0));

        // add F extra edges
        for (int i = 0; i < F; i++) {
            int v = edu.princeton.cs.algs4.StdRandom.uniform(V);
            int w = StdRandom.uniform(V);
            G1.addEdge(v, w);
            G2.addEdge(new DirectedEdge(v, w, 0.0));
        }

        edu.princeton.cs.algs4.StdOut.println(G1);
        edu.princeton.cs.algs4.StdOut.println();
        edu.princeton.cs.algs4.StdOut.println(G2);

        // find a directed cycle
        TopologicalX topological1 = new TopologicalX(G1);
        if (!topological1.hasOrder()) {
            edu.princeton.cs.algs4.StdOut.println("Not a DAG");
        }

        // or give topologial sort
        else {
            edu.princeton.cs.algs4.StdOut.print("Topological order: ");
